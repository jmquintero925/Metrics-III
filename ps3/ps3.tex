...
\todo[inline]{Okay I decided I have a gripe with the write-up. 

Recall that compliers refers to the group who change D in response to Z. Here, compliers refers to the group of suppliers who are influenced by $Z$. Since we assume that $k\neq 0$ is constant, this implies that the representative supplier is a complier. 

I don't think it makes sense to refer to buyers as compliers because they are not affected by the instrument directly. I have left further notes in a comment below this.}

\begin{comment} In this setup, Z is the instrument which affects P through the supply side. Since $k,\beta^S_1$ are constant this is a constant effects model which is like what Isaac said - `only one supplier or many identical suppliers'. 

So I can divide suppliers into four groups 1) Always 2) Never 3) Complier 4) Defier. Since $k$ is constant and positive, the representative supplier is a complier so we do not need to worry about the other groups.

Compliers refers to the group for whom D (here P) is moved by Z. Complier doesn't make sense to describe consumers here because they are not moved by Z, by assumption. They are not compliers, defiers, always, never-takers. 

Each consumer has her own price-elasticity. We are simply taking an average over these. Since we have assumed $\beta_i^d \geq 0$, everyone responds to the price change. 
\end{comment}
...
\input{ps3/tables/q4_partH_tab2}
